  ...
  todayTasks: protectedProcedure.query(async ({ ctx }) => {
    ...
    return rows.map((r) => {
      ...
      return {
        ...
        beneficiary,
        overdue,
        label: taskLabelFor(r.status, beneficiary),
        sub: taskSubFor(r.status),
      };
    });
  }),
...
function taskLabelFor(status: string, beneficiary: string | null): string {
  const who = beneficiary ?? "Unnamed beneficiary";
  switch (status) {
    case "documents_pending":
      return `Upload documents · ${who}`;
    case "draft_ready":
      return `Review drafts · ${who}`;
    case "needs_revision":
      return `Revisions needed · ${who}`;
    case "build_failed":
      return `Build failed · ${who}`;
    default:
      return who;
  }
}

function taskSubFor(status: string): string {
  switch (status) {
    case "documents_pending":
      return "Awaiting evidence uploads";
    case "draft_ready":
      return "Generated by Docket — awaiting your review";
    case "needs_revision":
      return "You marked outputs as needing changes";
    case "build_failed":
      return "Pipeline error — see case for details";
    default:
      return "";
  }
}
